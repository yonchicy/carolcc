...
op_v cal(NODE* base_node,std::vector<std::string> &cal_stream){
  ...
  if(arg[0].op.compare("")!=0){ WARNNING(" %s",arg[0].op.c_str());}
  else if(arg[0].lval.compare("")!=0){ WARNNING(" %s",arg[0].lval.c_str());}
  else{ WARNNING(" %d",arg[0].v);}
   WARNNING(" = %s",op.c_str());
  if(arg[1].op.compare("")!=0){ WARNNING(" %s,",arg[1].op.c_str());}
  else if(arg[1].lval.compare("")!=0){ WARNNING(" %s",arg[1].lval.c_str());}
  else{ WARNNING(" %d,",arg[1].v);}
  if(arg[2].op.compare("")!=0){ WARNNING(" %s\n",arg[2].op.c_str());}
  else if(arg[2].lval.compare("")!=0){ WARNNING(" %s\n",arg[2].lval.c_str());}
  else{ WARNNING(" %d\n",arg[2].v);}
